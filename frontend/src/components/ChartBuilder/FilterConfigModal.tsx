...
/** 数值比较符（枚举之外的单值比较）。 */
const NUMBER_COMPARE_OPERATORS: FilterOperator[] = ['gt', 'gte', 'lt', 'lte', 'eq', 'neq'];
/** 字符串比较符。 */
const STRING_COMPARE_OPERATORS: FilterOperator[] = ['eq', 'neq', 'like'];
/** 日期比较符。 */
const DATE_OPERATORS: FilterOperator[] = ['eq', 'neq', 'gt', 'gte', 'lt', 'lte', 'between'];
  ...
  useEffect(() => {
    ...
    if (k === 'date') {
      ...
      if (mode === 'enum') {
        ...
        setOperator(
          initOperator && NUMBER_COMPARE_OPERATORS.includes(initOperator) ? initOperator : 'gte'
        );
        setSingleNumber(init?.value != null && init.value !== '' ? Number(init.value) : null);
      }
    } else {
      // 新建条件默认枚举模式；仅当已有条件本身是比较符时才回显为条件模式。
      const mode: StringMode =
        !isNew &&
        init &&
        init.operator !== 'in' &&
        init.operator !== 'notIn' &&
        STRING_COMPARE_OPERATORS.includes(init.operator)
          ? 'compare'
          : 'enum';
      setStringMode(mode);
      if (mode === 'enum') {
        ...
        setOperator(
          initOperator && STRING_COMPARE_OPERATORS.includes(initOperator) ? initOperator : 'eq'
        );
          ...
          {numberMode === 'compare' && (
            <Space>
              <Select
                ...
                value={operator}
                onChange={setOperator}
                options={NUMBER_COMPARE_OPERATORS.map((op) => ({
                  value: op,
        ...
        {stringMode === 'enum' ? (
          ...
          <Space>
            <Select
              ...
              value={operator}
              onChange={setOperator}
              options={STRING_COMPARE_OPERATORS.map((op) => ({
                value: op,
                label: OPERATOR_LABELS[op],
              }))}
              data-testid="filter-modal-operator"
