 ...
 * ⚠️ 维度蓝取 antd 5/6 的 colorPrimary 色阶（#1677ff），不取旧版 #1890ff ——
 * 本页字段标签走的是 antd 预设色（blue=#1677ff），两个蓝并存会让"维度"出现两种蓝。
 */
const ZONE_PALETTE: Record<DropZoneType, { accent: string; hoverBackground: string }> = {
  dimension: { accent: '#1677ff', hoverBackground: '#e6f4ff' },
  metric: { accent: '#52c41a', hoverBackground: '#f6ffed' },
  filter: { accent: '#fa8c16', hoverBackground: '#fff7e6' },
};
...
export const dropZoneSurfaceStyle = (zoneType: DropZoneType, isOver: boolean): CSSProperties => {
  ...
  return {
    ...
    // 定义在 styles/index.css 的 .chart-builder-page）：槽位读起来是卡面上"挖出的坑"，
