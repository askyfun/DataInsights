...
func TestBuildPercentileExpr_Strategies(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name:     "quantilesExactInclusive p=0.5 中位数（CH，标量聚合形状）",
			field:    `"amount"`,
			p:        0.5,
			caps:     &datasource.DialectCapabilities{PercentileStrategy: "quantilesExactInclusive"},
			wantExpr: `quantileExactInclusive(0.5)("amount")`,
		},
		{
			name:     "quantilesExactInclusive p=0.25 Q1",
			field:    `x`,
			p:        0.25,
			caps:     &datasource.DialectCapabilities{PercentileStrategy: "quantilesExactInclusive"},
			wantExpr: `quantileExactInclusive(0.25)(x)`,
		},
		{
			name:        "window_ntile 无法作标量表达式，显式报错（架构性未实现）",
			field:       `x`,
			p:           0.5,
			caps:        &datasource.DialectCapabilities{PercentileStrategy: "window_ntile"},
			wantErrSubs: "not implementable as a scalar aggregate expression",
		},
		{
			name:        "未知策略视为配置错误",
			field:       `x`,
